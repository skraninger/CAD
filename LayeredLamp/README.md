...
- **`LedAttinyTouchx2.ino`** — 30-px strip with **two** touch pads: `TOUCH_COLOR` cycles the base colour, `TOUCH_MODE` cycles the pattern (8 total: solid, breathe, chaser, rainbow, wavey, scanner, firefly, wipe). Both settings persist in EEPROM (write deferred 5 s after the last touch to limit wear).
  - **Colour cycling:** each `TOUCH_COLOR` press advances `colorIndex` by 15 through **257** steps — wheel positions 0–255 plus **white** at index 256 (via the `baseColor()` helper). Since gcd(15, 257) = 1 (257 is prime), the cycle visits every step before returning to the start (~21° of hue per press). `colorWheel()` maps the wheel to a full rainbow in inverted order vs. Adafruit's standard: red → orange → yellow → green → cyan → blue → violet → magenta → pink → white → red.
  - The colour index is stored across two EEPROM bytes (`ADDR_COLOR` low + `ADDR_COLOR_HI` high, the high byte flagging white); a 0xFF high byte left by older firmware is treated as unwritten, so previously saved colours are kept.
  - The base colour only drives solid, breathe, and chaser; rainbow and the wavey/scanner/firefly/wipe patterns override it with their own colours.
