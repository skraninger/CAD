...
* **v15.1 (Current - Smooth Side→Top Shoulder):** Replaced the hard mesa-cap clamp (`np.where(Z_base > 9.2, ...)`) -- which left a ~0.2-unit vertical "spike" exactly where the cliff met the cap -- with a C1-smooth smoothstep shoulder that blends the cliff profile into the flat cap via `Z_base = lerp(Z_sig, cap_level, w_cap)`. Added a new `Side→Top Transition` slider (raw units, 0.2–8.0, default 1.5) that controls how gradual that handover is: small = tight rounded rim, large = the flat-cap surface bleeds gradually down the flank. Source file remains `GenerateButteV15.py`.

  * **v15.2 (Current - Clean Window Exit):** Fixed the program not exiting when the close X was clicked on Windows. Root cause: `plt.figure()` (pyplot) silently creates a second hidden Tk root window (`tk.Tk(className="matplotlib")`) inside matplotlib's figure manager; with two Tk instances in one process, `mainloop()` never returns after the app window is destroyed on Python 3.13, leaving a zombie process running forever. Fix: the preview canvas now uses a bare `matplotlib.figure.Figure` (no pyplot manager, no extra window), so the app's Tk root is the only instance and the process exits cleanly on close. Verified: pre-fix repro hung indefinitely; post-fix the full GUI shuts down in ~4s. Source file remains `GenerateButteV15.py`.

  * **v15.3 (Noise Seed & Mesh Simplification):** Added a UI Noise Seed spinbox (0-99999, default 55 = historical pattern) that drives `np.random.seed(int(seed))` before the fractal phase draws -- same seed => identical texture on every run, any other integer gives a new deterministic pattern. Added a Mesh Simplification control (integer 1-8, default 1 = full detail) applied to the exported STL only via `simplify_grid_mesh()`: keeps every N-th grid node per axis, cutting facet count by ~N² while preview/verify stay full detail. Source file remains `GenerateButteV15.py`.

  * **v15.4 (Current - Compact Side-by-Side GUI):** Rebuilt the control panel so every parameter is one row -- prompt on the left, control on the right (grid layout) -- halving the panel height and shrinking the window from 1100x1000 to 1100x780. Converted the exact-value parameters (Target Width, Target Height, Flat Contour Brim Width, Shell Thickness, Mesh Simplification) from sliders to number-entry spinboxes; the continuous "feel" parameters (Stratification, Texture Bottom/Sides/Top, Side→Top Transition) remain sliders and now show a live numeric value readout beside them. All control reads are centralized in `get_params()` with validation: invalid/empty entry text falls back to the default and out-of-range values clamp to the old slider bounds. Source file remains `GenerateButteV15.py`.
  ...
  *   **Clean Window Exit (v15.2):** The preview figure is a bare `matplotlib.figure.Figure`, NOT pyplot's `plt.figure()`. Pyplot's figure manager creates a second hidden Tk root window (`tk.Tk(className="matplotlib")`); with two Tk instances in one process, `mainloop()` never returns after the app window is destroyed on Python 3.13 (clicking X left a zombie process). The bare Figure keeps the app's Tk root as the sole instance so the program exits cleanly on close. Do not "simplify" this back to `plt.figure()`.
   ...
   *   **Compact Control Layout (v15.4):** The left control panel is a grid of one-row-per-parameter rows (prompt column + stretching control column), window geometry 1100x780. Exact mm values and integers are spinbox number entries; continuous factors are horizontal sliders with a live numeric value readout beside them. Entry text is validated on read (`get_params()`): bad/empty text falls back to the default, out-of-range values clamp to the control's min/max.
...
## 6. Current Parametric Controls & Initialization Benchmarks
...
*   **Target Width (`width_entry`, number entry since v15.4):** 20mm to 230mm boundary limit range (Defaults to **120.0 mm**).
*   **Target Height (`height_entry`, number entry since v15.4):** 20mm to 230mm range (Defaults to **120.0 mm**).
...
*   **Side→Top Transition (`side_top_slider`):** 0.2 to 8.0 raw-unit range (Defaults to **1.5**). Controls the width of the C1-smooth shoulder that blends the cliff profile into the flat mesa cap -- i.e. how gradual the side→top handover is. Small = tight rounded rim; large = the flat-cap surface bleeds gradually down the flank. Replaces the old hard cap clamp (Section 5B).
*   **Flat Contour Brim Width (`border_entry`, number entry since v15.4, step 0.5):** 0.0mm to 25.0mm range (Defaults to **5.0 mm** flat protective brim offset).
*   **Shell Thickness (`shell_entry`, number entry since v15.4, step 0.5):** 0.0mm to 20.0mm range (Defaults to **5.0 mm** open-bottom uniform shell thickness).
*   **Noise Seed (`seed_spin`, v15.3):** Integer spinbox, 0 to 99999 (Defaults to **55**, the historical pattern). Sets `np.random.seed(int(seed))` for the fractal texture phase draws -- same seed => identical texture on every run; any other integer gives a new deterministic pattern.
*   **Mesh Simplification (`simplify_entry`, v15.3):** Integer 1 to 8 (Defaults to **1** = full detail). Applied to the exported STL only via `simplify_grid_mesh()`: keeps every N-th grid node per axis, cutting facet count by ~N²; preview and verify stay full detail.
*   **Entry Validation (v15.4):** Number entries are read through `get_params()` / `_read_float()`: invalid or empty text falls back to the control's default and out-of-range values clamp to the min/max above, so the engine can never receive a value outside the old slider bounds. Sliders carry a live numeric readout label that updates as they move.
...
### File Layout
*   **Lines ~1-17 - Imports & Platform Hooks:** `matplotlib.use("TkAgg")` must stay directly below the imports (Windows thread-deadlock fix). The figure comes from `from matplotlib.figure import Figure` -- a bare `Figure`, never pyplot's `plt.figure()`, which spawns a second hidden Tk root and hangs process exit on close (v15.2 clean-exit fix; see Section 3).
*   **`generate_custom_border_butte(style, target_width_mm, target_height_mm, stratification_factor, texture_bottom, texture_sides, texture_top, side_top_transition, border_padding_mm, shell_thickness_mm, resolution=100, seed=55)` (~lines 22-303) - Core Engine.** Returns the 7-tuple `(X_scaled, Y_scaled, Z_scaled, X_in, Y_in, Z_in, valid_mask)`. Internal numbered steps: (1) radial framing + angular asymmetry, (2) sigmoid mesa profile + smooth side→top shoulder (slider-controlled), (3) fractal noise + zone texture field, geological style switcher, (4) valley-floor masking, (5) brim dilation (mm to pixel pad), (6) mm rescaling of X/Y/Z, (7) normal-extrusion hollowing.
*   **`calculate_normal(p1, p2, p3)` (~line 308):** per-facet unit normal with a `(0, 0, 1)` fallback for degenerate input.
*   **`simplify_grid_mesh(X, Y, Z, X_in, Y_in, Z_in, mask, stride)` (~line 315, v15.3):** coarsens the grid by keeping every `stride`-th node per axis (facet count drops ~N²); `stride <= 1` returns the input unchanged. The STL compiler re-derives outer/inner pairing from whatever mask it gets, so a downsampled mesh stays a closed open-bottom manifold shell.
*   **`write_contour_binary_stl(X, Y, Z, X_in, Y_in, Z_in, mask, shell_thickness_mm, filename)` (~lines 332-394) - STL Compiler:** outer-surface triangles (only cells whose four mask corners are all valid), inner-surface triangles with reversed winding (emitted only when shell > 0), perimeter stitch strips along mask transitions, degenerate-facet filter (area^2 > 1e-9), then 80-byte header + uint32 count + 50 bytes per facet.
*   **`ButteGeneratorGUI` (~lines 399-647) - Tkinter UI:** `add_slider_row()` / `add_entry_row()` row factories (each builds one prompt|control grid row and auto-binds approval invalidation), `get_params()` (single validated read of every control, in engine argument order -- preview/approve/export all consume it), `get_seed()` / `get_simplify()` (v15.3 controls), `update_preview()` (resolution 60), `update_plot_and_approve()` (resolution 80, unlocks export), `redraw_canvas()`, `save_stl()` (regenerates at resolution 130 and writes the file). Layout is one row per parameter -- prompt left, control right (v15.4); window geometry 1100x780. The preview figure is a bare `matplotlib.figure.Figure` embedded via `FigureCanvasTkAgg(..., master=plot_frame)` -- never a pyplot figure (v15.2 clean-exit fix).
...
2.  **Deterministic noise:** `np.random.seed(int(seed))` is set before phase draws, where `seed` is the UI Noise Seed control (v15.3; default 55 = historical pattern). The four frequency phases are drawn sequentially from this single stream, so inserting any random draw before the loop shifts every texture pattern in the model for every seed value. Use a separate `np.random.default_rng(...)` stream for new stochastic features instead.
...
9.  **Adding a control = 2 touch points (v15.4):** (a) one `add_slider_row(...)` or `add_entry_row(...)` line in `__init__` (auto-binds invalidation), (b) read it once inside `get_params()` -- preview, approve, and export all consume that single tuple (the v15.3 seed/simplify controls are read via `get_seed()` / `get_simplify()`). The engine's parameter order is fixed: style, width, height, strata, tex_bottom, tex_sides, tex_top, side_top_transition, border, shell, resolution, seed. Window geometry is currently 1100x780 for the compact one-row-per-parameter layout; each row is ~32px tall, so add height in ~32px steps if the panel outgrows it.
...
2. Windows Platform Fixes: Uses explicit 'matplotlib.use("TkAgg")' declarations right below imports to bypass Windows thread deadlock states. The preview figure is a bare 'matplotlib.figure.Figure', never pyplot's plt.figure() -- pyplot spawns a second hidden Tk root window, and with two Tk instances in one process mainloop() hangs on close (zombie process). Keep it that way so the program exits cleanly when the X button is clicked.
...
5. Controls: Includes number entries for target width and height (20-230mm range, both defaulting to 120.0mm), a rock style switcher, three zone texture sliders (bottom strata / mesa sides / mesa top, each 0.0-4.0, all defaulting to 1.5) that blend per-zone via a partition-of-unity weight field, a Side→Top Transition slider (0.2-8.0 raw units, default 1.5) that controls how gradual the C1-smooth cliff-to-cap shoulder is, number entries for flat contour brim offset (defaulting to 5.0mm) and shell thickness (defaulting to 5.0mm), a Noise Seed spinbox (default 55 = historical texture pattern; same seed => identical texture every run), and a Mesh Simplification integer entry (1-8, default 1 = full detail, applied to the exported STL only). The GUI is compact side-by-side rows (prompt left, control right); entries validate on read -- bad text falls back to defaults, out-of-range values clamp.
...
7. Reference Material: This profile contains a full code map, the critical invariants to respect before editing (raw-unit domain constants, UI-controlled RNG seed -- default 55 -- with load-bearing phase draw order, partition-of-unity zone weights, STL winding orientation, approval state machine), and a headless STL watertightness verification recipe in Sections 7-8. Read those sections before modifying the engine or the STL writer.
