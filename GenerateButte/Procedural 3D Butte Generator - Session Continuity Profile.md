...
*   **`ButteGeneratorGUI` (~lines 249-376) - Tkinter UI:** `create_slider()` factory (auto-binds approval invalidation), `update_preview()` (resolution 60), `update_plot_and_approve()` (resolution 80, unlocks export), `redraw_canvas()`, `save_stl()` (regenerates at resolution 130 and writes the file). The preview figure is a bare `matplotlib.figure.Figure` embedded via `FigureCanvasTkAgg(..., master=plot_frame)` -- never a pyplot figure (v15.2 clean-exit fix).
